...
def add_products():
    for _ in range(10):
        product = Product(
            name=f"{fake.color_name()}{random.randint(0,11)}",
...
def avg_ful_time():
    print('Average Fulfullment Time')
    print(
        db.session.query(
        db.func.time(
            db.func.avg(
...
def cust_purch_over_x(amount=500):
    print("All customers who have purchases over x dollars")
